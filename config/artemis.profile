...
# Java Opts
if [ -z "$JAVA_ARGS" ]; then
    JAVA_ARGS="-Djava.net.preferIPv4Stack=true -Xms131m -Xmx524m   -XX:MinHeapFreeRatio=10 -XX:MaxHeapFreeRatio=20 -XX:GCTimeRatio=4 -XX:AdaptiveSizePolicyWeight=90 -XX:MaxMetaspaceSize=100m -XX:ParallelGCThreads=1 -Djava.util.concurrent.ForkJoinPool.common.parallelism=1 -XX:CICompilerCount=2 -XX:+ExitOnOutOfMemoryError -XX:+PrintClassHistogram -XX:+UseG1GC  -Dhawtio.realm=activemq  -Dhawtio.offline=true -Dhawtio.role=app_q84_dev_r_amq,app_q84_dev_rw_amq,app_q84_dev_adm_amq -Dhawtio.rolePrincipalClasses=org.apache.activemq.artemis.spi.core.security.jaas.RolePrincipal -Djolokia.policyLocation=${ARTEMIS_INSTANCE_ETC_URI}jolokia-access.xml -Djon.id=amq"
fi
#
